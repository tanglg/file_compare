...
- **四档分析深度**：快速（Fast）、均衡（Balanced）、深度（Deep）、穷尽（Exhaustive），灵活权衡速度与召回范围
...
2. **选择分析深度**：
   - `快速`：速度优先，收紧召回范围，适合初步筛查
   - `均衡`（默认）：速度与精度均衡，日常推荐
   - `深度`：增加候选召回、细化文本切块，适合重点复核，耗时较长
   - `穷尽`：进一步放宽召回条件，适合最终审查，耗时最高且会产生更多需人工确认的弱候选
...
| `min_chunk_chars` | 最短文本块长度 | 80 |
| `target_chunk_chars` | 目标文本块长度 | 500 |
| `chunk_overlap_chars` | 相邻文本块重叠字符数 | 80 |
| `shingle_size` | Shingle 粒度（字符 n-gram） | 5 |
| `min_shared_shingles` | 最少共同 shingle 数（候选召回） | 3 |
| `simhash_hamming_threshold` | SimHash 最大汉明距离 | 4 |
| `candidate_score_threshold` | 候选召回分数阈值 | 0.35 |
| `candidate_top_k_per_file` | 每文件最多保留候选对数量 | 20 |
| `max_matches_per_pair` | 每对文件最多展示文本证据条数 | 30 |

高级参数可通过界面右上角"设置"面板调整。

检查深度的核心规律是：阈值越低、文本切块越细、块重叠越多、候选和证据上限越高，越容易发现零散复用或较弱相似；代价是执行时间增加，并且需要人工复核更多弱候选。
