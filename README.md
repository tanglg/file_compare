...
一款基于 **Tauri 2 + React + Rust** 的本地桌面应用，专为批量检测 PDF 文件之间的文本相似度而设计。所有计算均在本地完成，无需联网，数据不上传至任何服务器。
...
- **多维度相似度计算**：综合文本相似度、图像相似度与页面图像相似度三个维度，给出最终雷同分数
...
| 阶段 | 说明 |
|------|------|
| 快速扫描 | 逐页抽取 PDF 文本与文档元数据 |
| 全局索引 | 基于 Shingle 的文本倒排索引构建 |
| 候选召回 | 模板页降噪 + Top-K 候选截断 |
| 候选精算 | 精确页匹配与近似文本覆盖率计算 |
...
- 需安装 PDF 文件，纯图片扫描件效果有限（文本层为空时归类为 `text-empty`）
...
- 纯扫描 PDF（无文本层）相似度检测效果有限，建议使用带文字识别（OCR）的 PDF
